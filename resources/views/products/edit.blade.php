		...
		<div>
			<form action="{{ route('product.update', $product) }}" method="POST" enctype="multipart/form-data">
				...
				<div class="form-row">
					<div class="form-group col-12 col-md-6">
						...
						<div class="input-group">
							...
							<input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price', $product->price) }}" placeholder="0.00" required>
							@error('price')
								<div class="invalid-feedback">{{ $message }}</div>
							@enderror
						</div>
					...
					<div class="form-group col-12 col-md-6">
						...
						<input type="number" class="form-control @error('quantity') is-invalid @enderror" id="quantity" name="quantity" value="{{ old('quantity', $product->quantity) }}" placeholder="0" required>
						@error('quantity')
							<div class="invalid-feedback">{{ $message }}</div>
						@enderror
					</div>
				...
				<div class="form-group">
					...
					<textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4" placeholder="Product Description">{{ old('description', $product->description) }}</textarea>
					@error('description')
						<div class="invalid-feedback">{{ $message }}</div>
					@enderror
				</div>
				...
				<div class="form-row">
					<div class="form-group col-12 col-md-6">
						...
						<div id="featuredImagePreview" class="mt-3" style="display: none;">
							...
							<div class="position-relative d-inline-block">
								<img id="featuredImagePreviewImg" src="" alt="Preview" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">
								<button type="button" class="btn btn-danger btn-sm position-absolute" style="top: 5px; right: 5px;" onclick="removeFeaturedImage()">
									<i class="gd-close"></i>
								</button>
							</div>
						</div>
					...
					<div class="form-group col-12 col-md-6">
							...
							<div class="mb-2">
								...
								<div class="d-flex flex-wrap" id="existingGalleryContainer">
									@foreach($product->gallery as $index => $galleryImage)
										<div class="position-relative mr-2 mb-2" id="existing_gallery_{{ $index }}">
											<img src="{{ asset('storage/' . $galleryImage) }}" alt="Gallery" class="img-thumbnail" style="width: 100px; height: 100px; object-fit: cover;">
											<button type="button" class="btn btn-danger btn-sm position-absolute" style="top: -5px; right: -5px; padding: 2px 6px;" onclick="removeExistingGalleryImage({{ $index }}, '{{ $galleryImage }}')">
												<i class="gd-close"></i>
											</button>
										</div>
									@endforeach
						...
						<input type="file" class="form-control-file @error('gallery.*') is-invalid @enderror" id="gallery" accept="image/*" multiple>
						@error('gallery.*')
							<div class="invalid-feedback">{{ $message }}</div>
						@enderror
						<small class="form-text text-muted">Upload new images to replace current gallery. You can select multiple files or add one by one.</small>
